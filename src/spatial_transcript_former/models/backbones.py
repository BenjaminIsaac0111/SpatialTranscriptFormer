...
class ConvStem(nn.Module):
    ...
    def __init__(
        ...
        self.blob1 = nn.Sequential(
            nn.Conv2d(
                in_chans, l1_embed_dim, kernel_size=3, stride=2, padding=1, bias=False
            ),
        ...
        self.blob2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(
                l2_embed_dim, embed_dim, kernel_size=3, stride=1, padding=1, bias=False
            ),
    ...
    if name == "resnet50":
        ...
        if pretrained:
            try:
                ...
                print(
                    f"Downloading CTransPath weights (safetensors) from Hugging Face..."
                )
                ...
                print(
                    f"CTransPath weights loaded. Missing: {len(msg.missing_keys)}, Unexpected: {len(msg.unexpected_keys)}"
                )
        ...
        try:
            model = timm.create_model(
                "hf_hub:prov-gigapath/prov-gigapath", pretrained=pretrained
            )
            feature_dim = model.num_features
            if num_classes is not None:
        ...
        try:
            ...
            arch = (
                "vit_base_patch16_224" if name == "hibou-b" else "vit_large_patch14_224"
            )
        ...
        try:
            ...
            if pretrained:
                weights_path = hf_hub_download(
                    repo_id="owkin/phikon", filename="pytorch_model.bin"
                )
                state_dict = torch.load(
                    weights_path, map_location="cpu", weights_only=True
                )
                ...
                msg = model.load_state_dict(new_state_dict, strict=False)
                print(
                    f"Phikon weights loaded. Missing keys: {len(msg.missing_keys)}, Unexpected: {len(msg.unexpected_keys)}"
                )
        ...
        try:
            ...
            if pretrained:
                weights_path = hf_hub_download(
                    repo_id="vinid/plip", filename="pytorch_model.bin"
                )
                state_dict = torch.load(
                    weights_path, map_location="cpu", weights_only=True
                )
                ...
                if not new_state_dict:
                    for k, v in state_dict.items():
                        new_state_dict[
                            k.replace("module.visual.", "").replace("visual.", "")
                        ] = v

                msg = model.load_state_dict(new_state_dict, strict=False)
                print(
                    f"PLIP vision weights loaded. Missing keys: {len(msg.missing_keys)}, Unexpected: {len(msg.unexpected_keys)}"
                )
            ...
            print(
                f"Error loading PLIP: {e}. Note: PLIP might require `pip install transformers` for native loading."
            )
            raise e
        ...
        model = timm.create_model(
            name, pretrained=pretrained, num_classes=num_classes if num_classes else 0
        )
        feature_dim = model.num_features
        ...
        if model is None:
            try:
                ...
                raise NotImplementedError(
                    f"Backbone {name} not implemented or found: {e}"
                )

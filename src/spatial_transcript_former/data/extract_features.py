...
def extract_features_for_slide(
    ...
    model.eval()
    with torch.no_grad():
        for imgs, coords, barcodes in tqdm(
            loader, desc=f"Processing {os.path.basename(h5_path)}", leave=False
        ):
            imgs = imgs.to(device)
    ...
    torch.save(
        {"features": all_features, "coords": all_coords, "barcodes": all_barcodes},
        output_path,
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Root directory containing patches/ subdirectory",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--backbone", type=str, default="resnet50", help="Backbone model name"
    )
    parser.add_argument(
        "--batch-size", type=int, default=128, help="Inference batch size"
    )
    parser.add_argument(

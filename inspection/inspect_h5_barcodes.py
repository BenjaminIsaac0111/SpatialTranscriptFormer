...
def main():
    parser = argparse.ArgumentParser(
        description="Inspect barcodes in a HEST .h5 patch file."
    )
    parser.add_argument(
        "--id",
        type=str,
    ...
    if not file_path:
        print(
            f"Error: Could not find {sample_id}.h5 in any of the potential directories: {potential_dirs}"
        )
        return
    ...
    try:
        with h5py.File(file_path, "r") as f:
            ...
            for b in barcodes[:5]:
                ...
                val = (
                    b[0]
                    if hasattr(b, "__len__") and not isinstance(b, (bytes, str))
                    else b
                )

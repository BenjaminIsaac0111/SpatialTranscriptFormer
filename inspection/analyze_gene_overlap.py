...
def main():
    ...
    human_bowel = df[
        (df["organ"].str.lower() == "bowel") & (df["species"] == "Homo sapiens")
    ]
    ...
    print(
        f"Genes common to top 1000 of ALL {len(processed_ids)} samples: {len(intersection)}"
    )
    ...
    for sid in processed_ids:
        ...
        with h5py.File(h5ad_path, "r") as f:
            if "var" in f and "_index" in f["var"]:
                ...
                g = f["var"]["index"][:]
            all_genes_ref.append(
                set([x.decode("utf-8") if isinstance(x, bytes) else str(x) for x in g])
            )

...
import numpy as np
import pytest
from spatial_transcript_former.data.dataset import apply_dihedral_augmentation, apply_dihedral_to_tensor
...
def test_sync_logic():
    ...
    for op in range(8):
        ...
        if len(new_pos) > 0:
            ...
            print(f"Op {op} ({aug_img.shape}): Found pixel at ({nx}, {ny}) -> Rel=({rx}, {ry}) | Expected Coords=({ax}, {ay})")

...
class TestCheckpointRoundtrip:
    def test_save_load_preserves_weights(self, small_model, checkpoint_dir):
        ...
        # Save
        save_checkpoint(small_model, optimizer, None, epoch=42,
                        best_val_loss=0.123, output_dir=checkpoint_dir,
                        model_name='interaction')

        # Load into fresh model
        fresh_model = SpatialTranscriptFormer(
            num_genes=100, num_pathways=10, token_dim=64,
            n_heads=4, n_layers=1, use_nystrom=False) 
        fresh_optimizer = optim.Adam(fresh_model.parameters(), lr=1e-4)

        start_epoch, best_val = load_checkpoint(
            fresh_model, fresh_optimizer, None,
            checkpoint_dir, 'interaction', 'cpu'
        )
    ...
    def test_save_load_preserves_scaler(self, small_model, checkpoint_dir):
        ...
        save_checkpoint(small_model, optimizer, scaler, epoch=10,
                        best_val_loss=0.5, output_dir=checkpoint_dir,
                        model_name='interaction')
        ...
        load_checkpoint(fresh_model, fresh_optimizer, fresh_scaler,
                        checkpoint_dir, 'interaction', 'cpu')
    ...
    def test_no_checkpoint_starts_fresh(self, small_model, checkpoint_dir):
        ...
        optimizer = optim.Adam(small_model.parameters(), lr=1e-4)
        start_epoch, best_val = load_checkpoint(
            small_model, optimizer, None,
            checkpoint_dir, 'nonexistent', 'cpu'
        )
...
class TestArchitectureMismatch:
    def test_nystrom_checkpoint_fails_on_standard(self, small_nystrom_model, checkpoint_dir):
        """Nystrom checkpoint should fail to load into standard model."""
        optimizer = optim.Adam(small_nystrom_model.parameters(), lr=1e-4)
        save_checkpoint(small_nystrom_model, optimizer, None, epoch=5,
                        best_val_loss=0.3, output_dir=checkpoint_dir,
                        model_name='interaction')
        ...
        with pytest.raises(RuntimeError):
            load_checkpoint(standard_model, standard_optimizer, None,
                            checkpoint_dir, 'interaction', 'cpu')
    ...
    def test_matching_architecture_loads(self, small_nystrom_model, checkpoint_dir):
        ...
        fresh = SpatialTranscriptFormer(
            num_genes=100, num_pathways=10, token_dim=64,
            n_heads=4, n_layers=1, use_nystrom=True, num_landmarks=32)

"""
Tests for the Trainer class and callback system.

Uses a tiny synthetic dataset and model to verify the training lifecycle
...
class TestCallbacks:
    ...
    def test_early_stopping(self, tiny_setup):
        ...
        # Wrap so the model always outputs a near-constant → loss stays flat → early stop
...
class TestTrainerResume:
    ...
    def test_resume_continues_from_checkpoint(self, tiny_setup):
        ...
        # Resume — should start from epoch 3 and run to 5
        trainer2 = Trainer(
            model=model,
            train_loader=train_loader,
            val_loader=val_loader,

...
def check_alignment(sample_id='TENX29', output_path='visualizations/alignment_check.png'):
    ...
    with h5py.File(h5ad_path, 'r') as f:
        ...
        if 'obs' in f and '_index' in f['obs']:
            st_barcodes = [b.decode('utf-8') if isinstance(b, bytes) else str(b) for b in f['obs']['_index'][:]]
        else:
            st_barcodes = []
    ...
    ax.scatter(vis_coords[:, 0], vis_coords[:, 1], c='cyan', s=1, alpha=0.5, label='Patch Centers')
    
    # Highlight a few specific points to check orientation
    # Top-left most point
    tl_idx = np.argmin(coords[:, 0] + coords[:, 1])
    ax.scatter(vis_coords[tl_idx, 0], vis_coords[tl_idx, 1], c='red', s=50, edgecolors='white', label='Top-Left Point')
    
    # Bottom-right most point
    br_idx = np.argmax(coords[:, 0] + coords[:, 1])
    ax.scatter(vis_coords[br_idx, 0], vis_coords[br_idx, 1], c='yellow', s=50, edgecolors='black', label='Bottom-Right Point')

    ax.set_title(f"Alignment Check: {sample_id}\n(Red=TL, Yellow=BR) - Cyan points should form the tissue mask")
    ax.legend()

...
parser.add_argument('--run-dir', type=str, required=True, help="Path to the experiment run directory containing training_log.csv")
parser.add_argument('--port', type=int, default=8050, help="Port to host the dashboard on")
parser.add_argument('--interval', type=int, default=5000, help="Log polling interval in milliseconds")
...
app = dash.Dash(__name__, server=server, title=f"Training Monitor - {os.path.basename(args.run_dir)}")
...
app.layout = html.Div(style={'fontFamily': 'sans-serif', 'padding': '20px'}, children=[
    ...
    html.Div(id='last-updated', style={'color': 'gray', 'fontStyle': 'italic', 'marginBottom': '10px'}),
    html.Button('Pause Updates', id='pause-button', n_clicks=0, style={'marginBottom': '20px', 'padding': '10px', 'fontSize': '16px', 'cursor': 'pointer', 'backgroundColor': '#f0f0f0', 'border': '1px solid #ccc', 'borderRadius': '5px'}),
    
    html.Div([
        html.Label("Smoothing Window (Epochs):", style={'fontWeight': 'bold', 'marginRight': '10px'}),
        dcc.Slider(
            id='smoothing-slider',
            min=1,
            max=50,
    ...
    ], style={'marginTop': '40px', 'textAlign': 'center', 'backgroundColor': '#1a1a2e', 'padding': '20px', 'borderRadius': '10px'}),
...
def update_image(n):
    ...
    if not list_of_files:
        return html.P("No inference plots found yet. Make sure to run training with --plot-pathways.", style={'color': 'red'})
...
def update_graphs(n, smoothing_window):
    if not os.path.exists(log_path):
        return dash.no_update, dash.no_update, "Waiting for training_log.csv to be created..."
    ...
    if df.empty or 'epoch' not in df.columns:
         return dash.no_update, dash.no_update, "Log file is empty or missing 'epoch' column."
    ...
    loss_layout = go.Layout(
        ...
        yaxis=dict(title='Loss', type='log'), # Log scale often helps with early MSE spikes
    ...
    for col in metric_cols:
        ...
        metric_traces.append(go.Scatter(
            x=df['epoch'],
            y=y_data,
            mode='lines',
            name=col
    ...
    return {'data': loss_traces, 'layout': loss_layout}, {'data': metric_traces, 'layout': metric_layout}, update_text
...
def toggle_pause(n_clicks):
    base_style = {'marginBottom': '20px', 'padding': '10px', 'fontSize': '16px', 'cursor': 'pointer', 'borderRadius': '5px', 'border': '1px solid #ccc'}
    if n_clicks % 2 == 1:
        # Paused state
        active_style = {**base_style, 'backgroundColor': '#ffcccc', 'borderColor': '#ff0000'}

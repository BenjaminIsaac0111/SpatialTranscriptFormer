...
def load_model(checkpoint_path, args, device):
    ...
    if "pathway_tokenizer.pathway_embeddings" in state_dict:
        _, num_pathways, token_dim = state_dict[
            "pathway_tokenizer.pathway_embeddings"
        ].shape
    else:
        num_pathways = 50
        token_dim = 512
...
def evaluate_sample(
    model, sample_id, args, device, membership, gene_names, pathway_names
):
    """Run inference and compute metrics for a single sample."""
    # Paths
    feat_dir_name = (
        "he_features" if args.backbone == "resnet50" else f"he_features_{args.backbone}"
    )
    ...
    print(
        f"  Preds:   Min={preds.min():.4f}, Max={preds.max():.4f}, Mean={preds.mean():.4f}"
    )
    print(
        f"  Targets: Min={targets.min():.4f}, Max={targets.max():.4f}, Mean={targets.mean():.4f}"
    )
...
def main():
    ...
    parser.add_argument(
        "--mode", type=str, default="both", choices=["genes", "pathways", "both"]
    )
    ...
    if num_pathways != membership.shape[0]:
        print(
            f"WARNING: Model was trained with {num_pathways} pathways but we parsed {membership.shape[0]}."
        )
    ...
    for sid in tqdm(val_ids):
        metrics = evaluate_sample(
            model, sid, args, device, membership, gene_names, pathway_names
        )
        if metrics:

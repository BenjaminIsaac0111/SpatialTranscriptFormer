...
def get_sample_ids(data_dir, filter_organ=None):
    ...
    if os.path.exists(metadata_path):
        ...
        if filter_organ:
            ...
            df_final = df_human[
                df_human["organ"].str.contains(filter_organ, case=False, na=False)
            ]
        else:
            df_final = df_human
...
def calculate_global_genes(data_dir, ids, num_genes=1000, target_pathways=None):
    ...
    if target_pathways:
        ...
        # Load C2 KEGG first
        kegg_path = download_msigdb_gmt(
            MSIGDB_URLS["c2_kegg"],
            MSIGDB_URLS["c2_kegg"].split("/")[-1],
            os.path.join(data_dir, ".cache"),
        )
        dict_kegg = parse_gmt(kegg_path)

        kegg_med_path = download_msigdb_gmt(
            MSIGDB_URLS["c2_medicus"],
            MSIGDB_URLS["c2_medicus"].split("/")[-1],
            os.path.join(data_dir, ".cache"),
        )
        dict_med = parse_gmt(kegg_med_path)

        # Load C2 CGP
        cgp_path = download_msigdb_gmt(
            MSIGDB_URLS["c2_cgp"],
            MSIGDB_URLS["c2_cgp"].split("/")[-1],
            os.path.join(data_dir, ".cache"),
        )
        dict_cgp = parse_gmt(cgp_path)

        # Also load hallmarks just in case
        h_path = download_msigdb_gmt(
...
def main():
    ...
    parser.add_argument(
        "--organ", type=str, default=None, help="Filter by organ (e.g., Bowel)"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="List of MSigDB pathway names to explicitly include",
    )

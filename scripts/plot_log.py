...
def plot_training_log(csv_path, output_path):
    ...
    metric_cols = [
        c for c in df.columns if "pcc" in c.lower() or "pcc_mean" in c.lower()
    ]
    lr_cols = [c for c in df.columns if "lr" in c.lower()]

    fig, axes = plt.subplots(
        2 if len(metric_cols) > 0 else 1, 1, figsize=(10, 10), squeeze=False
    )
...
if __name__ == "__main__":
    csv_file = (
        r"z:\Projects\SpatialTranscriptFormer\runs\stf_pathway_hybrid\training_log.csv"
    )

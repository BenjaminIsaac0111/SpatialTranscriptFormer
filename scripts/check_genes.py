...
print(
    f"Overlap with sum-top {num_genes} global genes: {len(overlap)} / {len(unique_hallmark_genes)} ({len(overlap)/len(unique_hallmark_genes)*100:.1f}%)"
)
...
if os.path.exists(test_file):
    ...
    with h5py.File(test_file, "r") as f:
        ...
        if isinstance(X, h5py.Group):
            ...
            print(
                f"Mean expression: {dense_sub.mean():.4f}, Max: {dense_sub.max():.4f}"
            )
